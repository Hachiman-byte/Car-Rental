<?php  
	...
	if (isset($_GET['update'])) {
		...
		$update = "UPDATE brands set brand_name = '{$name}' where brand_id = '{$fr_id}'";
...
<body>
	...
	<div class="tablebox">
			<p class="info">EDIT BRAND</p>
			
					<?php  
						if (isset($_GET['edit'])) {
							$fr_id = $_GET['edit'];
							$sql_show = "SELECT * from brands where brand_id = '{$fr_id}'";
					...
					<form method="get" class="upcategory">
						<div>
							...
							<input type="number" hidden="" name="id" value="<?php echo $rows['brand_id']; ?>">
							<input type="text" name="catname" value="<?php echo $rows['brand_name']; ?>">
						</div>
